	...
	<?php 
	    $sqlData=mysql_query("SELECT date_data FROM fbs_default_date order by id DESC limit 1");
		$dateRow=mysql_fetch_assoc($sqlData);
		
        $sql2=mysql_query("select count(id) from student_info_fbs WHERE payment_status='0' AND challan_date='".$dateRow['date_data']."'");
   ...
   <table class="table table-striped table-bordered table-hover table-responsive"   width="100%">
                                ...
                                <tr style="font-size:10px;">
                                  ...
                                  <tbody>
                                    <?php
                                    $i=1; 
                                    $sql=mysql_query("select * from student_info_fbs WHERE payment_status='0' AND challan_date='".$dateRow['date_data']."' order by id ASC  $limit") or die(); 
